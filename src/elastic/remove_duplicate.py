...
keys_to_include_in_hash = ["short_id"]
...
def scroll_over_all_docs():
    data = es.search(index="guardian", scroll='1m',  body={"query": {"match_all": {}}})
    # Get the scroll ID
    sid = data['_scroll_id']
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
      if len(array_of_ids) > 1:
        ...
        matching_docs = es.mget(index="guardian", doc_type="_doc", body={"ids": array_of_ids})
        first = True
        for doc in matching_docs['docs']:
            ...
            print("doc=%s\n" % doc)
            if not first:
                es.delete(index="guardian", id=doc['_id'])
            first = False
