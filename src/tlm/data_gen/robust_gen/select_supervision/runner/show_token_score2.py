...
from misc_lib import tprint, two_digit_float, average
from tlm.data_gen.robust_gen.select_supervision.token_score import token_score_by_ablation, SegmentScorePair, \
    AnalyzedDoc
...
def main():
    ...
    step_size = 16
    window_size = 128
    out_entries: List[AnalyzedDoc] = token_score_by_ablation(info_file_path,
    ...
    for e in out_entries:
        max_score: float = max(lmap(SegmentScorePair.get_max_score, flatten(e.token_info.values())))
        if max_score < 0.6:
            if n_neg > n_pos:
                continue
            else:
        ...
        for idx in range(score_len):
            ...
            full_scores: List[float] = lmap(SegmentScorePair.get_score_diff, token_info)
