...
from tlm.data_gen.robust_gen.robust_generators import RobustTrainGenSelected
...
def main():
    ...
    for target_selection in ["all", "best", "first_and_best", "best_or_over_09"]:
        worker_factory = partial(RobustWorkerWDataID,
                                 RobustTrainGenSelected(encoder, max_seq_length, score_d, "desc", target_selection))
        runner = JobRunner(job_man_dir, 3, "robust_selected_{}".format(target_selection), worker_factory)
        runner.start()
