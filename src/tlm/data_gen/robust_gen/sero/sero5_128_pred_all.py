...
from tlm.data_gen.adhoc_datagen import MultiWindowAllSeg


def generate_robust_sero_for_train():
    total_sequence_length = 128 * 4
    src_window_size = 128
    encoder = MultiWindowAllSeg(src_window_size, total_sequence_length)
    worker_factory = partial(RobustWorker, RobustPredictGen(encoder, total_sequence_length, 100, "desc"))
    runner = JobRunner(job_man_dir, 4, "RobustSero5_128_pred_all_seg", worker_factory)
    runner.start()
