...
from misc_lib import DataIDManager, pick1, TimeEstimator, TEL, Averager
...
class PredictionGenBasedLengthStat:
    def __init__(self, resource: ProcessedResourcePredict,
                 basic_encoder,
                 max_seq_length):
        self.resource = resource
        ...
        for qid in TEL(qids):
            ...
            for doc_id in self.resource.candidate_doc_d[qid]:
                ...
                try:
                    ...
                    for passage_idx, passage in enumerate(insts):
                        tokens_seg, seg_ids = passage
                        maybe_doc_part_len = len(tokens_seg) - len(q_tokens) - 3
                        averager.append(maybe_doc_part_len)
