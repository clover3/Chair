...
def independent_model(loss1, loss2, mask):
    loss_list = []
    for y1, y2 in zip(loss1, loss2):
        ...
        pred_prob2 = random.random() * gold_prob1
...
def run(filename, n_item):
    ...
    for idx, features in enumerate(load_record2(filename)):
        ...
        loss_list.append(independent_model(loss1, loss2, mask))
        loss_list2.append(diff_model(loss1, loss2, mask))

    print("independent : ", average(loss_list))
