...
from galagos.types import SimpleRankedListEntry, RankedListDict
...
def filter_by_doc_id(ranked_list_d: RankedListDict,
    ...
    for q_id, ranked_list in ranked_list_d.items():
        ...
        for idx, entry in enumerate(filtered_entry):
            new_list.append(SimpleRankedListEntry(doc_id=entry.doc_id, rank=idx + 1, score=entry.score))
        new_d[q_id] = new_list
...
def main():
    ...
    ranked_list: Dict[str, List[SimpleRankedListEntry]] = load_galago_ranked_list(ranked_list_path)
