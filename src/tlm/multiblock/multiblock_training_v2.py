...
import models.transformer.bert_common_v2 as bert_common
...
flags.DEFINE_float("lr_factor", 10, "The initial learning rate for Adam.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      ) = bert_common.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
      if use_tpu:
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      train_op = optimization.create_optimizer_different(
          total_loss, name_d, lr_factor, learning_rate, num_train_steps, num_warmup_steps, use_tpu)
...
def gather_indexes(sequence_tensor, positions):
  ...
  sequence_shape = bert_common.get_shape_list(sequence_tensor, expected_rank=3)
