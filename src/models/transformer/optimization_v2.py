...
def create_optimizer_different(loss, name_d, factor, init_lr, num_train_steps, num_warmup_steps, use_tpu):
  ...
  if use_tpu:
    optimizer1 = tf.compat.v1.tpu.CrossShardOptimizer(optimizer1)
    optimizer2 = tf.compat.v1.tpu.CrossShardOptimizer(optimizer2)

  tvars = tf.compat.v1.trainable_variables()
  var_list1 = list([t for t in tvars if not name_d(t.name) ])
  var_list2 = list([t for t in tvars if name_d(t.name)])
  ...
  train_op1 = optimizer1.apply_gradients(zip(grads1, var_list1), global_step=global_step)
  train_op2 = optimizer2.apply_gradients(zip(grads2, var_list2), global_step=global_step)
