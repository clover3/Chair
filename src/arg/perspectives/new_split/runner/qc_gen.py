...
from arg.perspectives.new_split.common import get_qids_for_split, split_name1, \
...
def main():
    save_dir = os.path.join(output_path, "pc_qc3")
    exist_or_mkdir(save_dir)
    split_filename = split_name1
    for split in splits:
        qids: Iterable[str] = get_qids_for_split(split_filename, split)
        queries = get_qck_queries_from_cids(lmap(int, qids))
        eval_candidate: Dict[str, List[QCKCandidate]] = get_qck_candidate_for_split(split_filename, split)
