...
def get_flat_score_save_path(save_name):
    exist_or_mkdir(path_join(output_path, TASK_NAME))
    exist_or_mkdir(path_join(output_path, TASK_NAME,  "prediction_scores_flat",))
    save_path = path_join(output_path, TASK_NAME, "prediction_scores_flat", save_name + ".txt")
    return save_path
