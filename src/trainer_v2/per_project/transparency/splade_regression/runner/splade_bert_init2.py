...
from trainer_v2.custom_loop.modeling_common.bert_common import load_bert_config, load_stock_weights_encoder_only, \
    load_stock_weights_bert_like, load_params
...
from trainer_v2.per_project.transparency.splade_regression.modeling.bert_sparse_encoder import BertSparseEncoder, BertLMLogits
...
def name_mapping(name):
    name = name.split(":")[0]
    if name == "cls/predictions/output_weights":
        return "bert/embeddings/word_embeddings"
    return name


class BERT_VR2(ModelV2IF):
    ...
    def build_model(self, run_config):
        ...
        self.inner_model = BertLMLogits(bert_params, self.dataset_info)
    ...
    def init_checkpoint(self, init_checkpoint):
        if init_checkpoint is None:
            ...
            load_params(self.inner_model.lm_params, init_checkpoint, name_mapping, 6)
...
def main(args):
    ...
    trainer: TrainerIF = TrainerVectorRegression(run_config, BERT_VR2(dataset_info))
