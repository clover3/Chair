...
def draw_chart(labels, values):
    ...
    plt.gca().xaxis.set_tick_params(labeltop=True, labelbottom=False, top=True, bottom=False)  # Move x labels to the top
...
def main():
    ...
    for method_name in sota_retrieval_methods:
        ...
        plt = draw_chart(a_to_z[:10], change_value[:10])
        img_save_path = path_join(output_path, "mmp", "append_test", method_name + "_blue.png")
