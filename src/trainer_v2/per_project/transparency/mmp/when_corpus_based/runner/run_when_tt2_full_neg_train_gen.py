...
class Worker(WorkerInterface):
    def __init__(self, out_dir):
        ...
        fn = feature_encoder.get_tt_weight_feature_from_tfs
        self.out_dir = out_dir
        self.encode_fn = get_encode_fn_when_bow(fn)
        self.pos_neg_sampler = MMPPosNegSampler()

    def enum_pos_neg(self, job_id):
        file_path = path_join(output_path, "msmarco", "passage", "when_full_re_tokenized", str(job_id))
        itr = load_pickle_from(file_path)
    ...
    def work(self, job_no):
        ...
        n_item = 1000 * 1000
        write_records_w_encode_fn(save_path, self.encode_fn, itr, n_item)


def main():
    working_dir = path_join("output", "msmarco", "passage")
    runner = JobRunnerS(working_dir, 17, "train_when_tt2_full_neg", Worker)
    runner.start()
