...
def save_term_effect(te_list: Iterable[TermEffectPerQuery], q_term, d_term, job_no):
...
def term_effect_serial_core(sm, q_term, d_term):
    n_job = 0
    n_qd = 0
    n_query = 0
    st = time.time()
    for job_no in get_mmp_split_w_deep_scores():
        ...
        save_term_effect(out_itr, q_term, d_term, job_no)
