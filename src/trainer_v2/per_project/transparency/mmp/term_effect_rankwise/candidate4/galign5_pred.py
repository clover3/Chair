...
def get_matching_terms_fn(batch_size, model, top_k):
    def get_matching_terms(q_term: List[int]) -> List[Tuple[int, float]]:
        d_term_id_st = 1000
        d_term_id_ed = 3000
        d_term_list = list(range(d_term_id_st, d_term_id_ed))
        eval_dataset = build_dataset_q_term_d_term_multi(q_term, d_term_list)
        batched_dataset = eval_dataset.batch(batch_size)
        outputs = model.predict(batched_dataset)
        scores = outputs['align_pred'][:, 0]
...
def main():
    ...
    st = n_per_job * job_no
    ed = st + n_per_job
    save_path = path_join(output_path, "msmarco", "passage", "candidate4", f"{job_no}.jsonl")
    f = open(save_path, "w")
