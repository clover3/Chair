...
class TransformerSelfAttentionLayerWMask(Layer):
    class Params(ProjectionLayer.Params,
                 AttentionLayerWMask.Params):
        hidden_size         = None
        num_heads           = None
        hidden_dropout      = None
    ...
    def build(self, input_shape):
        ...
        self.attention_layer = AttentionLayerWMask.from_params(
            self.params,
        ...
        super(TransformerSelfAttentionLayerWMask, self).build(input_shape)
    ...
    def call(self, inputs, attention_mask, mask=None, training=None):
        ...
        #
        attention_head   = self.attention_layer(layer_input, attention_mask, mask=mask, training=training)
...
class SingleTransformerEncoderLayerWMask(Layer):
    """
    ...
    class Params(TransformerSelfAttentionLayerWMask.Params,
    ...
    def build(self, input_shape):
        ...
        self.self_attention_layer = TransformerSelfAttentionLayerWMask.from_params(
            self.params,
            name="attention"
        )
        ...
        super(SingleTransformerEncoderLayerWMask, self).build(input_shape)

    def call(self, inputs, attention_mask, mask=None, training=None):
        layer_input = inputs

        attention_output    = self.self_attention_layer(layer_input, attention_mask, mask=mask, training=training)
...
class TransformerEncoderLayerWMask(Layer):
    """
    ...
    class Params(SingleTransformerEncoderLayerWMask.Params):
        num_layers     = None
    ...
    def build(self, input_shape):
        ...
        if self.params.shared_layer:
            # ALBERT: share params
            self.shared_layer = SingleTransformerEncoderLayerWMask.from_params(self.params, name="layer_shared")
        else:
            # BERT
            for layer_ndx in range(self.params.num_layers):
                encoder_layer = SingleTransformerEncoderLayerWMask.from_params(
                    self.params,
                    name="layer_{}".format(layer_ndx),
                )
                self.encoder_layers.append(encoder_layer)

        super(TransformerEncoderLayerWMask, self).build(input_shape)
    ...
    def call(self, inputs, attention_mask, mask=None, training=None):
        ...
        for layer_ndx in range(self.params.num_layers):
            ...
            layer_output = encoder_layer(layer_input, attention_mask, mask=mask, training=training)
